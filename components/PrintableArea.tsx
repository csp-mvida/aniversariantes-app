...
import BirthdayTable from './components/BirthdayTable'; // Certifique-se de que o caminho está correto
...
const PrintableArea: React.FC<PrintableAreaProps> = ({ birthdays, month, year }) => {
  // Altura reservada para o rodapé (aproximadamente 100px ou 25 unidades tailwind)
  const FOOTER_HEIGHT_CLASS = 'pb-[100px]'; 

  return (
    <div 
      className="a4-container bg-white w-[210mm] h-[297mm] py-8 px-8 shadow-2xl border border-gray-200 flex flex-col items-center relative"
      id="printable-content"
    >
      {/* Conteúdo Principal (Header, Calendar, Table) */}
      <div className={`w-full flex flex-col items-center flex-grow ${FOOTER_HEIGHT_CLASS}`}>
        ...
        <section className="w-[85%] flex-grow overflow-y-auto">
      ...
      <footer 
        className="absolute bottom-0 left-0 right-0 flex flex-col items-center w-full py-4 bg-white"
      >
        <div className="w-[85%] h-[1px] bg-gray-100 mb-4"></div>
        <img 
