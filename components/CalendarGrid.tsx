...
const CalendarGrid: React.FC<CalendarGridProps> = ({ month, year, birthdays }) => {
  ...
  for (let i = 0; i < firstDayOfMonth; i++) {
    days.push(<div key={`pad-${i}`} className="aspect-square bg-gray-50/30 border border-gray-100"></div>);
  }
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    days.push(
      ...
      >
        <span className={`text-sm font-bold ${hasBirthday ? 'scale-110' : ''}`}>
          {d}
        </span>
      </div>
    );
    ...
    <div className="w-full text-xs">
      <div className="grid grid-cols-7 border-b mb-1" style={{ borderColor: COLORS.dark }}>
        {WEEKDAYS.map(day => (
          <div 
            key={day} 
            className="text-center py-1.5 font-black"
